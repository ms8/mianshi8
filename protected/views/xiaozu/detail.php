<div class="top3a"> <a class="fl" href="<?php echo $this->createUrl('/xiaozu/detail',array('id'=>$model->id)); ?>"><img src="<?php echo $model->imgLink; ?>"  width="48" height="48"/></a>
...
<div class="con clear">
    <div class="leftx">
        <div class="leftxa">
            <div class="lxat">创建于<?php echo date('Y-m-d',$model->create_time) ?>    创建人： <a href="<?php echo $this->createUrl('/kongjian/index',array('uid'=>$model->memberOne->id)); ?>"><?php echo $model->memberOne->nickname; ?></a> </div>
            <div class="lxatcon"> 
                <?php echo $model->des; ?>
            </div>
            <!-- <div> <a href="#">推荐</a> </div> -->
        ...
        <div class="huati">
            <div class="huati1"><a <?php if(empty($_GET['order'])){?>class='htjd' <?php }?> href="<?php echo Yii::app()->createUrl('xiaozu/detail',array('id'=>$model->id)); ?>">最近话题</a> / <a <?php if(!empty($_GET['order'])){?>class='htjd' <?php }?>  href="<?php echo Yii::app()->createUrl('xiaozu/detail',array('id'=>$model->id,'order'=>'hot')); ?>">最热话题</a></div>
        ...
        <div class="huatilist">
            ...
            <?php foreach ($topic as $key => $value) {?>
                <div class="huatilist1">
                    <div class="huatilist2"><a href="<?php echo Yii::app()->createUrl('xiaozu/topic',array('id'=>$value->id)); ?>"><?php  echo $value->title ?><?php if($value->status > 2){ echo "<b style='color:red;'>($value->statusName)</b>"; }; ?></a></div>
...
<!--         <div class="right6">友情小组</div>
        ...
        <?php if($model->mark==2){?>
            <div class="xiaozu2"> > <a href="<?php echo $this->createUrl('/xiaozu/chengyuan',array('gid'=>$model->id)); ?>">成员管理 (<?php echo $model->mmemberCount  ?>)</a> <br />> <a href="<?php echo $this->createUrl('/xiaozu/update',array('id'=>$model->id)); ?>">小组管理</a> <br />
        </div> 
        <?php  } ?>
...
<script>
    $('.addGroup').click(function(){
       ...
       var node =$(this);
           if(gid!='' && mid!=''){
                $.ajax({
                type: "POST",
                url: "<?php echo Yii::app()->createUrl('/xiaozu/add'); ?>",
                dataType:'json',
    ...
    $('.delGroup').click(function(){
       ...
       var mid  = <?php echo Yii::app()->user->id; ?>;
           if(gid!='' && mid!=''){
                $.ajax({
                type: "POST",
                url: "<?php echo Yii::app()->createUrl('/xiaozu/del'); ?>",
                dataType:'json',
                data: "gid="+gid+"&mid="+mid,
                success: function(data){
                  if(data.status==1){
    ...
    $('#fayan').click(function(){
        if(action){
             location.href = "<?php echo Yii::app()->createUrl('xiaozu/addtopic',array('id'=>$model->id)); ?>";
        }else{
