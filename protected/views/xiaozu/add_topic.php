    ...
    <!--     <div class="right6">最近加入</div>
        <div class="xiaozu">
          <?php foreach ($newMember as $key => $value) {?>
            ...
            <?php }  ?> -->
        <div class="right6"> <a href="<?php echo Yii::app()->createUrl('xiaozu/detail',array('id'=>$group->id)); ?>">返回>> <?php echo $group->name;  ?></a></div>
 ...
 $('#addtopic_group').ajaxForm({
    ...
    success:function processJson(data) {
      ...
      if(data.status != 1){       
        ...
        alert('提交成功');
        location.href = "<?php echo Yii::app()->createUrl('xiaozu/detail'); ?>/"+data.id;
      }
    }
  });
$("#sub").click(
